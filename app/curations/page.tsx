...
// Cover images cycled across folder cards when no item image is available
const COVER_FALLBACKS = [
  "/assets/stitch/stitch-33.jpg",
  "/assets/stitch/stitch-42.jpg",
  "/assets/stitch/stitch-37.jpg",
  "/assets/stitch/stitch-35.jpg",
];
...
export default function CurationsPage() {
  ...
  async function fetchFolders() {
    setLoading(true);
    const res = await get<WishlistFolder[]>("/api/wishlist/folders");
    if (res.ok) setFolders(res.data);
    setLoading(false);
  }

  async function handleCreateCuration() {
    const name = window.prompt("Name your new curation:");
    if (!name || !name.trim()) return;
    setCreating(true);
    const res = await post<WishlistFolder>("/api/wishlist/folders", { name: name.trim() });
    if (res.ok) setFolders((prev) => [...prev, res.data]);
    setCreating(false);
  }

  function handleViewCollection(folderId: string) {
    router.push(`/wishlist?folder=${folderId}`);
  }

  const totalItems = folders.reduce((sum, f) => sum + f.item_count, 0);
  ...
  return (
    <>
      ...
      <main className="pt-32 pb-section-gap px-margin-desktop max-w-container-max mx-auto min-h-screen">
        ...
        <section className="flex flex-col md:flex-row md:items-end justify-between gap-8 mb-16">
          ...
          <button
            ...
            disabled={creating}
            className="bg-primary-container text-pure-white px-8 py-4 rounded-lg font-label-caps text-label-caps uppercase tracking-widest flex items-center gap-3 hover:opacity-90 active:scale-95 transition-all shadow-lg disabled:opacity-60"
          >
            <span className="material-symbols-outlined" data-icon="add_circle">add_circle</span>
            {creating ? "Creating..." : "Create New Curation"}
          </button>
        </section>

        <div className="flex items-center justify-between border-b border-surface-container-high pb-4 mb-10">
          <div className="flex gap-8">
            <button className="font-label-caps text-label-caps text-pure-white border-b border-pure-white pb-4">
              {loading ? "All Folders" : `All Folders (${folders.length})`}
            </button>
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-gutter">
            {folders.map((folder, idx) => {
              const cover = COVER_FALLBACKS[idx % COVER_FALLBACKS.length];
              const countLabel = String(folder.item_count).padStart(2, "0");
              return (
                <div
                  key={folder.id}
                  className="group curation-card relative bg-surface-deep rounded-xl overflow-hidden aspect-[4/5] flex flex-col transition-all duration-500 hover:ring-2 hover:ring-primary-container/50"
                >
                  <div className="h-full w-full relative">
                    ...
                    <div className="absolute top-4 left-4">
                      <span className="bg-pure-white text-pitch-black font-label-caps text-[10px] px-3 py-1 rounded-full uppercase">{countLabel} Items</span>
                    </div>
                  </div>
                  <div className="absolute bottom-0 left-0 right-0 p-8 transform translate-y-4 group-hover:translate-y-0 transition-transform duration-500">
                    <h3 className="font-display-lg-mobile text-display-lg-mobile text-pure-white uppercase mb-2">{folder.name}</h3>
                    <div className="flex items-center gap-4 mt-6 opacity-0 group-hover:opacity-100 transition-all duration-500 delay-100">
                      <button
                        onClick={() => handleViewCollection(folder.id)}
                        className="bg-pure-white text-pitch-black px-4 py-2 rounded font-label-caps text-[10px] uppercase font-bold hover:bg-primary-container hover:text-pure-white transition-colors"
                      >
                        View Collection
                      </button>
            ...
            {/* New Vault / create shortcut card */}
            <div
              onClick={handleCreateCuration}
              className="group cursor-pointer border-2 border-dashed border-surface-container-highest rounded-xl flex flex-col items-center justify-center p-12 text-center aspect-[4/5] hover:border-primary-container hover:bg-surface-container-low transition-all duration-300"
            >
        ...
        {!loading && folders.length === 0 && (
          <p className="font-body-md text-body-md text-silver-mist text-center mt-8">No curations yet. Create your first vault above.</p>
        )}
      </main>
      ...
      <button
        onClick={handleCreateCuration}
        disabled={creating}
        className="fixed bottom-10 right-10 w-16 h-16 bg-primary-container text-pure-white rounded-full flex items-center justify-center shadow-2xl hover:scale-110 active:scale-95 transition-all z-40 md:hidden disabled:opacity-60"
      >
